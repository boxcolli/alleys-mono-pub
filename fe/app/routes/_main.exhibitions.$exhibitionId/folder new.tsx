...
export function Folder() {
  ...
  return (
    <Suspense>
      <Await resolve={Promise.all([myFolders, addedFolderIds])}>{([myFolders, addedFolderIds]) => {
        ...
        const addFolderSubmit = (folderId: string) => {
          addFolderFetcher.submit(
            ...
            {
              method: "post",
              action: $PATH.api.folders.$fid(folderId),
        ...
        useEffect(() => {
          ...
          const { folder_id: fid } = parsed.data

          // setFolders(prev => ({
          //   ...prev,
          //   [fid]: { ...prev[fid], isAdded: true },
          // }))
        }, [deleteFolderFetcher])
