...
import api from "../api.signup.resend-email/config"
...
export default function Page() {
  ...
  const resendEmail = useDebounce(() => fetcher.submit(
    new FormData(), { action: $PATH.api.auth.signup.resendEmail, method: "post" }),
    100,
  )
  const isResendSubmitting = fetcher.state !== "idle"
  ...
  useEffect(() => {
    ...
    // Handle resend error
    const parsed = api.res.schema.safeParse(fetcher.data)
    if (parsed.error) {
    ...
    // Reset state
    if (timerInit < timerMax) {
      setExpired(false)
    }

    deleteInput()
    // console.log({ timerInit, timerMax, expired })
  }, [fetcher.state, expired])
